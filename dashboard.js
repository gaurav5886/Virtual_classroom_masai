...
function renderClassLists() {
    ...
    if (upcomingClasses.length === 0) {
        ...
        upcomingClasses.slice(0, 3).forEach(classItem => {
            upcomingClassesList.appendChild(createClassCard(classItem, 'upcoming'));
        });
    }
}
...
function createClassCard(classItem, type) {
    ...
    classCard.className = `class-card ${type}`;

    const classTime = classItem.dateTime.toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit'
    });

    const classDate = classItem.dateTime.toLocaleDateString([], {
        weekday: 'short',
        month: 'short',
        day: 'numeric'
    });
    ...
    classCard.innerHTML = `
        <div class="class-card-header">
            ...
            <div class="class-card-time">${classDate} · ${classTime}</div>
        </div>
        <div class="class-card-instructor">Instructor: ${classItem.instructor}</div>
    `;

    // Add click event to show class details
    classCard.addEventListener('click', () => showClassDetails(classItem));
...
function setupEventListeners() {
    ...
    scheduleClassForm.addEventListener('submit', (e) => {
        ...
        const newClass = {
            ...
            duration,
            description,
            instructor: localStorage.getItem('userEmail') || 'Unknown Instructor',
            participants: [localStorage.getItem('userEmail') || 'Unknown User']
        };
    ...
    window.addEventListener('click', (e) => {
        if (e.target === scheduleClassModal) {
            scheduleClassModal.style.display = 'none';
        }
    });
}
